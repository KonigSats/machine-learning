...
void pdf(double* z, int n){
	for (int i=0;i<n;i++){
		...
		if(z[i] == 0.0) z[i] = 0.001;
		if(z[i] == 1.0) z[i] = 0.999;
	}
}
...
void performKmean(int nrows, int ncols, double** data, int** mask){
	...
	//cluster_matrix[clusterCounter++] = cluster1;
	//cluster_matrix[clusterCounter++] = cluster2;
	...
	printf("\n=====================test scores===================");
	for(int k=0;k<2;k++)
		printf("Score %d: %f", k, testScore[k]);
